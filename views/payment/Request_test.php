...
$type  = $_GET['p_type'];//json_decode($_GET['p_type']);       // 결제방식
echo $type;
$p_noti = "";
...
<form name="payForm" method="post" accept-charset="euc-kr">
	<table style="width:100%;" class="option-table">
		...
		<input type="hidden" name="P_UNAME" value="<?php echo $u_name ?>" placeholder="Hongildong">
		...
		<input type="hidden" name="P_MOBILE" value="<?php echo $u_phone ?>" placeholder="010-1234-5678" >
		...
		<input type="hidden" name="P_RETURN_URL" id="return" value="payment://" >
...
<script type="text/javascript" language="javascript">
	...
	*/
	...
	function reserved_change(){
		...
		var rVal = "twotrs_isp=Y&block_isp=Y&twotrs_isp_noti=N&hpp_corp=KTF:LGT:MVNO&cp_yn=Y&apprun_check=Y&below1000=Y&Twotrs_bank=Y&app_scheme=payment://&";

	    if ( type == "card" ) //신용카드
	       rVal = "twotrs_isp=Y&block_isp=Y&twotrs_isp_noti=N&cp_yn=Y&apprun_check=Y&below1000=Y&app_scheme=payment://&";
	    else                  // 계좌이체
	       rVal = "Twotrs_bank=Y&apprun_check=Y&app_scheme=payment://&iosapp=Y&app_scheme=payment://&";
