...
router.all("/add-product", async (req, res) => {
    try {
         ...
         * 픽업/배일일 후 3일후
         */
        let ended_date = new Date( req.body.finish_date );
         ...
         * 공구종료일 + 정산기간
         */
        let settlement_date = new Date( ended_date );
        // 공휴일수 계산
        let day = new Date( ended_date ).getDay();
        let array1 = [ 0, 5, 6 ]; // 일, 금, 토
        let count = 0;
    ...
    try {
        ...
        if ( keyword ) {
            ...
            for ( let k = 0; k < temp.salesHistories.length; k ++ ) {
                ...
                if ( ( item.buyer_phone && item.buyer_phone === keyword ) ||
...
router.all("/update-product", async (req, res) => {
    try {
        ...
        if ( temp.state === 1 ) {
            ...
            // 앵콜외쳐요로 공구 수정하기
            let angcolUsers = await Angcols.findAll({
                where: {
                    product_id: Number( req.body.product_id ),
                    deleted: null,
                },
            ...
            // if ( angcolUsers.length === 0 )
            //     return res.status(200).json({ status: false, msg: ['아직 앵콜외침한 사용자가 없습니다']});
            const { msg, isValid } = await validateRegisterProducts(req.body);
            if (!isValid) {
                return res.status(200).json({status: false, msg: msg});
            }
            ...
            let tokens = [];
            console.log("공구did:", angcolUsers);
            for ( let i = 0; i < angcolUsers.length; i ++ ) {
                await Alarms.create({
                    user_id: Number( angcolUsers[ i ].user_id ),
                    product_role: "product",
                ...
                if ( angcolUsers[ i ].angcolUser.push_token && angcolUsers[ i ].angcolUser.tool_notification )
                    tokens.push( angcolUsers[ i ].angcolUser.push_token );
...
router.all("/payment", async ( req, res) => {
    try {
        ...
        if ( !temp )
            ...
            let content = req.body.jwt_data.name + "님이 " + ( Number( req.body.payment_amount || 0 ) + Number( req.body.discount || 0 ) ).toLocaleString('ko') + "원 공구 결제했어요.";
            await Alarms.create({
            ...
            if ( temp.saleUser.push_token && temp.saleUser.tool_notification ) {
                const push_message = {
                    type: "PAYMENT",
                    title: title,
                    body: content,
...
router.all("/cancel-payment2", async ( req, res) => {
    try {
        ...
        await Users.update({
            points: user.points + Number( SalesHistories.discount ),
        }, {
            where: { id: req.body.jwt_data.id }
        });
...
router.all("/call-angcol", async ( req, res) => {
    try {
        ...
        if ( user_angcol )
            return res.status(200).json({ status: false, msg: ['고객님은 이 공구에 이미 앵콜외침을 하시였습니다']});
         ...
         */
        let title = "[공구마켓] " + temp.name;
        let content = req.body.jwt_data.name + "님이 판매자님의 공동구매를 원해요.";
        await Alarms.create({
            user_id: Number( temp.user_id ),
            product_role: "product",
            product_id: Number( req.body.product_id ),
        ...
        if ( temp.saleUser.push_token && temp.saleUser.angol_notification ) {
            const push_message = {
                type: "ANGCOLS",
                title: title,
                body: content,
...
router.all("/together-participate", async ( req, res ) => {
    try {
        ...
        let content = req.body.jwt_data.name + "님이 함께해요!를 참여했어요.";
        ...
        if ( token && sale_user.comment_notification ) {
            const push_message = {
                type: "COMMIT",
                title: title,
                body: content,
...
router.all("/together-fall", async ( req, res ) => {
    try {
        ...
        let content = req.body.jwt_data.name + "님이 함께해요!를 포기했어요.";
        ...
        if ( token && sale_user.tool_notification ) {
            const push_message = {
                type: "TOGETHER FALL",
                title: title,
                body: content,
...
router.all("/get-list-home", async ( req, res ) => {
    try {
        ...
        const total_pages = req.body.page ? Math.ceil(temp.length / parseInt(req.body.page_size)) : 1;
        ...
        return res.status(200).json({ status: true, results: temp, "got_count":len, "total_pages" : total_pages});
    } catch (e) {
        return res.status(200).json({ status: false, msg: [ e.toString() ]});
    }
});
